...
* **US-24 (Backup Dati Locale Flessibile):** Come utente, voglio poter salvare un backup dei miei dati utilizzando, dove supportato, la funzione di condivisione nativa del mio dispositivo per inviarlo ad altre app o salvarlo in una posizione a mia scelta.
* **US-25 (Ripristino da Backup Locale):** Come utente, voglio poter caricare un file di backup (`.json`) precedentemente salvato per ripristinare il mio `configUrl` e il mio `weeklyPlan`, con una richiesta di conferma prima di sovrascrivere i dati esistenti.
